...
PATTERN_OPTIONAL = {"id", "endpoint", "payout", "notes", "tags", "session_id", "environment", "target_type", "outcome", "context", "signals"}
PATTERN_ALL = PATTERN_REQUIRED | PATTERN_OPTIONAL

# Fields concatenated (in order) to form the embedding text for RAG.
# Change this list if you add richer free-text fields later.
# Format used at embed time:
#   "[vuln_class] [technique] | context: [context] | signals: [signals joined] | notes: [notes]"
RAG_TEXT_FIELDS = ("vuln_class", "technique", "context", "signals", "notes")
...
# CTF / live environment tagging (notes.txt §5)
VALID_ENVIRONMENTS = {"ctf", "live"}
# Broad target archetypes — used to weight strategy transfer (notes.txt §4)
VALID_TARGET_TYPES = {"bank", "saas", "auth_provider", "api_gateway", "oss", "ecommerce", "fintech", "telecom", "other", "ctf_challenge"}
# Outcome vocabulary for strategy weight updates (notes.txt §4)
VALID_OUTCOMES = {"no_effect", "informational", "duplicate", "valid", "high_impact"}
# Techniques that are safe in CTF but BLOCKED on live programs (notes.txt §5)
CTF_ONLY_TECHNIQUES = {"brute_force", "credential_flood", "endpoint_flood", "data_manipulation", "destructive"}
...
def validate_pattern_entry(entry: dict) -> dict:
    ...
    if "id" in entry:
        if not isinstance(entry["id"], str) or not entry["id"].strip():
            raise SchemaError("Pattern entry: 'id' must be a non-empty string")

    if "session_id" in entry:
        if not isinstance(entry["session_id"], str) or not entry["session_id"].strip():
            raise SchemaError("Pattern entry: 'session_id' must be a non-empty string")

    if "environment" in entry and entry["environment"] not in VALID_ENVIRONMENTS:
        raise SchemaError(
            f"Pattern entry: 'environment' must be one of {sorted(VALID_ENVIRONMENTS)}, got {entry['environment']!r}"
        )

    if "target_type" in entry and entry["target_type"] not in VALID_TARGET_TYPES:
        raise SchemaError(
            f"Pattern entry: 'target_type' must be one of {sorted(VALID_TARGET_TYPES)}, got {entry['target_type']!r}"
        )

    if "outcome" in entry and entry["outcome"] not in VALID_OUTCOMES:
        raise SchemaError(
            f"Pattern entry: 'outcome' must be one of {sorted(VALID_OUTCOMES)}, got {entry['outcome']!r}"
        )

    if "signals" in entry:
        if not isinstance(entry["signals"], list) or not all(isinstance(s, str) for s in entry["signals"]):
            raise SchemaError("Pattern entry: 'signals' must be a list of strings")
